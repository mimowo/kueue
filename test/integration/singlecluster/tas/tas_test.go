...
var _ = ginkgo.Describe("Topology Aware Scheduling", ginkgo.Ordered, func() {
	...
	ginkgo.When("Single TAS Resource Flavor", func() {
		...
		ginkgo.When("ProvisioningRequest is used", func() {
			...
			ginkgo.It("should admit workload when nodes are provisioned; only-limits requested by Workload; with restart", framework.SlowSpec, func() {
				var (
					wl1 *kueue.Workload
				)
				...
				ginkgo.By("Await for scheduler to be considered by scheduler", func() {
					util.ExpectReservingActiveWorkloadsMetric(clusterQueue, 1)
				})

				for _, node := range nodes {
					ginkgo.By(fmt.Sprintf("make the node %s Ready", node.Name), func() {
						createdNode := &corev1.Node{}
