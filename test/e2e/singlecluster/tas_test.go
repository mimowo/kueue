	...
	ginkgo.When("Creating a Job With Queueing", func() {
		...
		ginkgo.It("should admit a Job via TAS", func() {
			...
			sampleJob = (&testingjob.JobWrapper{Job: *sampleJob}).
				PodAnnotation(kueuealpha.PodSetTopologyRequiredAnnotation, topologyLevel).
			...
			ginkgo.By("await for admission and verify TopologyAssignment", func() {
						...
						Domains: []kueue.TopologyDomainAssignment{
							{
								...
								Levels: []kueue.TopologyDomainAssignmentLevel{
									{
										NodeLabel:      topologyLevel,
										NodeLabelValue: "kind-worker",
									},
								},
							},
						},
			...
			ginkgo.By("verify the workload gets finished", func() {
				gomega.Eventually(func() bool {
					if err := k8sClient.Get(ctx, wlLookupKey, createdWorkload); err != nil {
						return false
					}
		...
		ginkgo.FIt("should allow to evict a TAS Job", func() {
			...
			ginkgo.By("verify the high-priority workload gets finished", func() {
