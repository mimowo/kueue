...
func CreateAndStartVisibilityServer(ctx context.Context, kueueMgr *qcache.Manager, enableInternalCertManagement bool, kubeConfig *rest.Config, port int) error {
	config := newVisibilityServerConfig(kubeConfig)
	if err := applyVisibilityServerOptions(config, enableInternalCertManagement, port); err != nil {
...
func applyVisibilityServerOptions(config *genericapiserver.RecommendedConfig, enableInternalCertManagement bool, port int) error {
