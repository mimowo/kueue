...
func newRfReconciler(c client.Client, queues *queue.Manager, cache *cache.Cache, recorder record.EventRecorder) *rfReconciler {
	return &rfReconciler{
		log:      ctrl.Log.WithName("tas-resource-flavor"),
		client:   c,
		queues:   queues,
		cache:    cache,
...
func (r *rfReconciler) setupWithManager(mgr ctrl.Manager, cache *cache.Cache, cfg *configapi.Configuration) error {
...
func (h *nodeHandler) queueReconcileForNode(node *corev1.Node, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	...
	for _, rf := range h.tasCache.GetResourceFlavors() {
...
func (r *rfReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if flv.Spec.TopologyName != nil {
		if r.tasCache.Get(kueue.ResourceFlavorReference(flv.Name)) == nil {
			...
			for lk, lv := range flv.Spec.NodeLabels {
				req, err := labels.NewRequirement(lk, selection.Equals, []string{lv})
				if err != nil {
					return reconcile.Result{}, err
				}
			...
			tasInfo := r.tasCache.NewCacheRF(levels, flv.Spec.NodeLabels)
...
func (r *rfReconciler) Update(event event.UpdateEvent) bool {
	...
	if isOldRf && isNewRf {
		if ptr.Equal(oldRf.Spec.TopologyName, newRf.Spec.TopologyName) {
			return false
		}
		if ptr.Equal(oldRf.Spec.TopologyName, newRf.Spec.TopologyName) {
			return false
		} else if oldRf.Spec.TopologyName == nil {
			// topologyName is added
			return true
		} else {
